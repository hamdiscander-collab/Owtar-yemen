...
export default async function handler(req, res) {
    ...
    // سحب المفتاح الجديد الذي اخترته HF_Log
    const HF_SECRET_KEY = process.env.HF_Log; 

    if (!HF_SECRET_KEY) {
        return res.status(500).json({ error: "المفتاح HF_Log غير موجود في Vercel" });
    }
    ...
    try {
        ...
        res.status(500).json({ error: "حدث خطأ فني في الجسر البرمجي" });
    }
}
